...
describe('BusinessLandingPage', () => {
  beforeEach(() => TestBed.configureTestingModule({ imports: [BusinessLandingPage], providers: [provideRouter([])] }));
  ...
  it('configures every primary action and SEO metadata', () => {
    ...
    expect(document.querySelector('meta[name="description"]')?.getAttribute('content')).toContain('Organizá tu agenda');
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toContain('/para-negocios');
  });
});
